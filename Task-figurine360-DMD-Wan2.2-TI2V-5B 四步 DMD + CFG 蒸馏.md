# Task-figurine360-DMD - Wan2.2-TI2V-5B 四步 DMD + CFG 蒸馏（ 共三阶段）

> 执行者：Codex + GPT-5.6 Sol。总原则：**能复用参考仓库已有实现的一律复用，所有改动原地修改（in-place）、加配置开关、默认关闭、不做无关重构**。每个改造点独立 commit，附 2-cycle 小分辨率冒烟测试。
...
基座：`quanhaol/Wan2.2-TI2V-5B-Turbo` main 分支（已拉取至当前目录下）。**选择理由**：
...
- 官方产物：HF `quanhaol/Wan2.2-TI2V-5B-Turbo` 的 `model.pt`（≈20GB 全量 generator 权重）——本任务用作 LoRA 初始化源（5.3）。
...
**Step 0 必须确认的四件事**（读代码即可，不用跑训练）：
 ...
 ② `dmd.sh` 的 generator 初始化来源（从 base 直接启动 / 需要 ODE-init checkpoint / 加载 Turbo model.pt），据实调整我们的启动方式，若依赖 ODE-init 则评估直接从 merged teacher 启动的可行性；
...
max_iters: 3000                    # generator 步数；有 Turbo LoRA/delta-SVD 初始化则降到 1000
ema_decay: 0.995
...
一点提醒：kijai 那个 rank-64 LoRA 是社区提取物，其提取时的 alpha 约定和 key 命名未必与我们 5.3 的 peft/手写 LoRALinear 一致，所以文档里我把它定位为"初始化首选但 key 对齐后才用、失败不阻塞"，最终产物仍以我们自己训练导出的 LoRA 为准。

------

参考：

1. quanhaol/Wan2.2-TI2V-5B-Turbo（GitHub，训练/推理入口与环境）：https://github.com/quanhaol/Wan2.2-TI2V-5B-Turbo
2. quanhaol/Wan2.2-TI2V-5B-Turbo（HF，model.pt 权重）：https://huggingface.co/quanhaol/Wan2.2-TI2V-5B-Turbo
3. guandeh17/Self-Forcing（上游框架，configs 语义 / warp_denoising_step / denoising_loss_type；注意其主线是因果学生，我们只用其基础设施）：https://github.com/guandeh17/Self-Forcing
4. yetter-ai/Wan2.2-TI2V-5B-Turbo-Diffusers（Turbo 可作为标准双向模型加载的证据）：https://huggingface.co/yetter-ai/Wan2.2-TI2V-5B-Turbo-Diffusers
5. Kijai/WanVideo_comfy 的 Wan22-Turbo 目录（Turbo rank-64 LoRA 提取产物，5.3 初始化首选）：https://huggingface.co/Kijai/WanVideo_comfy
6. lightx2v/Wan2.2-Lightning（LoRA 初始化备选源 / Phased DMD 备选方案）：https://github.com/ModelTC/Wan2.2-Lightning
7. Wan-Video/Wan2.2（TI2V-5B 官方推理行为对齐基准）：https://github.com/Wan-Video/Wan2.2